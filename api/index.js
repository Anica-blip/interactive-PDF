...
async function healthCheck(req, res) {
  try {
    let wasabiStatus = 'not configured';
    ...
    if (s3Client) {
      try {
        ...
        wasabiStatus = 'error: ' + error.message;
      }
    }
    ...
    return res.status(200).json({
      ...
      config: {
        ...
        hasCredentials: !!(wasabiConfig.accessKeyId && wasabiConfig.secretAccessKey)
      }
    });
  } catch (error) {
...
async function generatePDF(req, res) {
  try {
    ...
    // Try to upload to Wasabi if configured
    let browserUrl = null;
    if (s3Client) {
      try {
        const cloudFilename = `${wasabiConfig.defaultFolder}/${uniqueId}.pdf`;
        await uploadToWasabi(pdfBuffer, cloudFilename);
        browserUrl = `${wasabiConfig.endpoint}/${wasabiConfig.bucketName}/${cloudFilename}`;
      } catch (uploadError) {
        console.error('Wasabi upload failed:', uploadError);
